...
def display_data(output_path: pathlib.Path | None, csv_file_path: pathlib.Path) -> None:
    ...
    speed = np.round(np.linalg.norm(np.column_stack((ball_dx, ball_dy)), axis=1) / np.diff(timestamps), 3)
    max_speed = np.max(speed)

    plt.figure(figsize=(10, 5))
    plt.plot(timestamps[:-1], speed, label="Vitesse totale de la balle")
    plt.plot(timestamps[:-1], speed, 'ro')
    plt.plot(timestamps[1:-1], np.diff(speed) / np.diff(timestamps[:-1]), label="Accélération de la balle")
    plt.fill_between(timestamps[:-1], 0, max_speed, where=(game_is_running[:-1] == 0), color='red', alpha=0.3, label="Jeu en pause")
    plt.xlabel("Temps (s)")
    plt.ylabel("Vitesse (m/s) et Accélération (m/s²)")
